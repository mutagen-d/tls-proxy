...
/**
 * @typedef {{
 *  pack: (...args: any) => Buffer
 *  unpack: (buffer: Buffer) => any
 * }} IPacker
 */

/**
 * @typedef {IPacker & { isTLSOptions?: boolean }} IExtraOptions
 */
...
class SocketBase {
   ...
   * @param {boolean | IExtraOptions} [extraOptions]
   */
  constructor(socketOrOptions, extraOptions = false) {
    let isTLSOptions = typeof extraOptions === 'object' ? extraOptions.isTLSOptions : extraOptions
    isTLSOptions = typeof isTLSOptions === 'boolean' ? isTLSOptions : false
    extraOptions = extraOptions || {}
    /** @private */
    this._pack = extraOptions && typeof extraOptions === 'object' && extraOptions.pack || packer.pack
    /** @private */
    this._unpack = extraOptions && typeof extraOptions === 'object' && extraOptions.unpack || packer.unpack
  ...
  onData(data) {
    try {
      const [event, ...args] = this._unpack(data)
  ...
  emit(event, ...args) {
    ...
    const data = this._pack(event, ...args)
